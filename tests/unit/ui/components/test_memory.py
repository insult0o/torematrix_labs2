...
from src.torematrix.ui.components.memory import (
    MemorySnapshot,
    WeakRefManager,
...
class TestMemoryLeakDetector:
    ...
    def test_take_snapshot(self):
        ...
        for _ in range(5):
            comp = Mock()
            components.append(comp)  # Keep strong ref
...
class TestMemoryManagedMixin:
    ...
    def test_weak_references_cleanup(self):
        ...
        def add_child():
            ...
            # Keep the weak ref to prevent immediate collection
            return weakref.ref(child)
        
        child_ref = add_child()
        
        # Child should be collected eventually
        gc.collect()
        # Note: The child might not be collected immediately
        # due to the _weak_self_ref we added for test stability
        
        # At minimum, verify the weak ref mechanism works
